...
func (d *DB) SaveOrder(ctx context.Context, co orderparam.SaveOrder) error {
	...
	_, err := d.conn.Conn().ExecContext(ctx,
		...
		generateNewID(), co.UserID, co.ProductCode, co.CustomerFullName, co.ProductName, co.TotalAmount)
